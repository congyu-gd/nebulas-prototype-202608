...
/* ------------------------------------------------------------------ web pages
   What the embedded browser in the results column shows for a URL a result
   carries — the page as generated, with real content: the watchlist as the
   Churn program publishes it, the case study as it reads on the site. Keyed by
   the bare address; a URL with no entry gets a modest page built from its
   address in app.js. `stats` are [value, label]; `table.meter` names the column
   drawn as a bar; `accent` is a DESIGN_ACCENTS name, confined to the page. */
const WEB_PAGES = {
  'churn.acme.app/watchlist':{
    site:'Acme · Customer programs', nav:['Watchlist','Playbooks','History'], accent:'Blue',
    title:'Account watchlist',
    sub:'Ranked by churn probability, next quarter · rebuilt this morning · owner joined from the CRM',
    stats:[['10','accounts on the list'],['2','moved up this week'],['0.81','highest signal']],
    table:{ head:['Account','Churn signal','Owner'], meter:1, rows:[
      ['Northwind Traders', 0.81, 'Dana Cole'],
      ['Contoso Retail',    0.74, 'Priya Nair'],
      ['Fabrikam',          0.63, 'Jon Alvarez'],
      ['Tailspin Toys',     0.61, 'Dana Cole'],
    ...
    body:['Scores come from accounts_health and rebuild on every run of the Churn program. A row is a conversation to have, not a verdict — the playbook for each band is under Playbooks.'],
    foot:'Generated by the Churn program · reads accounts_health'
  },
  'acme.com/stories/northwind':{
    site:'Acme Industrial', nav:['Stories','Machines','Service','Contact'], accent:'Emerald',
    title:'The line we did not replace',
    sub:'Northwind Traders · retrofit case study · 6 min read',
    stats:[['−31%','energy per unit'],['9 days','downtime, not 11 weeks'],['40 yrs','the frames we kept']],
    body:[
      'Northwind\'s stamping line is older than most of the people who run it. The brochures said replace it; the numbers said otherwise. We kept the frames, changed the drives, and rewired the controls cabinet in nine days of scheduled downtime.',
      'Energy per unit fell 31% in the first month and has held. The line now reports on itself — every drive publishes torque and temperature — which is how the maintenance plan moved from calendar to condition.',
      'What did not work: the first vibration sensors were mounted on painted surfaces and read noise for a week. The full write-up includes the mounting spec that fixed it.'
    ],
    cta:'Talk to the retrofit team'
  },
  'acme.com/openday':{
    site:'Acme Industrial', nav:['Stories','Machines','Service','Contact'], accent:'Emerald',
    title:'Open day — Rotterdam floor',
    sub:'Thursday 12 September · 09:00–16:00 · Waalhaven Zuidzijde 21',
    body:[
      'We are opening the Rotterdam floor for a day. Bring a line problem you have not solved and an engineer who has tried — the morning is demonstrations, the afternoon is your problems on our test benches.',
      'Lunch is on the floor, among the machines. Places are limited to keep the benches useful.'
    ],
    cta:'Reserve a place'
  }
};
...
/* --------------------------------------------------------- the page wizard
   Add → Page opens a guided chat: these are its questions, asked one at a
   time, in this order. The order IS the logic — nothing branches on an
   answer; a step that only applies sometimes says so in its own words
   ("...or say none"). `pre` is the acknowledgment said before the next
   question; `ph` is the input placeholder for a typed answer. The worked
   example is a monthly data analysis report: the goal is that running the
   project's workflow each month is all it takes to get the latest report. */
const PAGE_WIZARD = {
  steps:[
    { ask:'What should this page be? One line is enough — the steps after this collect the specifics.',
      opts:['A monthly data analysis report — the latest month, generated by running the project'],
      ph:'Describe the page…' },
    { pre:'A report page, then — numbers first, notes after.',
      ask:'Where does its data come from?',
      opts:['Local — tables already on the project shelf',
            'An external source, over an API',
            'Both — shelf tables joined with an external source'] },
    { pre:'Noted.',
      ask:'If an external source is involved it needs credentials — name the API and the account to use, or say none.',
      opts:['None — local data only',
            'Stripe · api.stripe.com · key on file',
            'Google Analytics · service account'],
      ph:'API endpoint · account…' },
    { pre:'That settles what the page reads.',
      ask:'What should the report carry?',
      opts:['Headline KPIs · monthly trend · breakdown table · written notes',
            'KPIs and the trend only',
            'One long-form written analysis'] },
    { pre:'Good — that is the whole shape.',
      ask:'Should a workflow rebuild it on a schedule, so running the project is all it takes to get the latest month?',
      opts:['Yes — monthly, first Monday at 08:00',
            'Yes — monthly, on the 1st at 09:00',
            'No — I will run it by hand'] },
    { pre:'Last thing.',
      ask:'What should the page be called?',
      opts:['Monthly data analysis report'],
      ph:'A name…' }
  ],
  build:[
    { n:'scaffold',      d:'monthly-report/ — the folder and its four files', t:'0.7s' },
    { n:'code.write',    d:'build_report.py — pull, compute, render the month', t:'1.2s' },
    { n:'workflow.wire', d:'workflow.yml — monthly schedule, retries, publish', t:'0.8s' },
    { n:'page.render',   d:'index.html from the latest data — first build', t:'1.0s' }
  ],
  done:[
    '**The page is built.** It lives as a folder holding everything it needs:',
    '',
    '- <code>index.html</code> — the layout the data pours into',
    '- <code>build_report.py</code> — pulls the sources, computes the month, renders the page',
  ...
  /* The folder's working files — what the Script tab and the artifact's Files
     tab both show. Short and real: each file says what a real one would. */
  files:[
    { nm:'index.html', sub:'layout',
      code:[
        '<!-- monthly-report/index.html — layout only; numbers arrive from build_report.py -->',
        '<main class="report">',
        '  <header>',
        '    <h1>{{ report.title }}</h1>',
        '    <p>{{ report.month }} · built {{ report.built_at }}</p>',
        '  </header>',
        '  <section class="kpis">{% for k in report.kpis %}',
        '    <div class="kpi">{{ k.value }}<span>{{ k.label }}</span></div>{% endfor %}',
        '  </section>',
        '  <section class="trend">{{ charts.monthly_trend }}</section>',
        '  <section class="table">{{ tables.breakdown }}</section>',
        '  <section class="notes">{{ report.notes }}</section>',
        '</main>'
      ].join('\n') },
    { nm:'build_report.py', sub:'the logic',
      code:[
        '# monthly-report/build_report.py — one run = one month, rendered.',
        'from nebulas import shelf, page',
        '',
        'def build(month=None):',
        '    month = month or latest_closed_month()',
        "    rows  = shelf.table('q3_ledger').month(month)   # local, per sources.json",
        '    kpis  = compute_kpis(rows)          # revenue, delta, top mover',
        '    trend = monthly_trend(rows, back=12)',
        '    notes = writeup(kpis, trend)        # short, numbers-first',
        "    page.render('index.html', kpis=kpis, trend=trend,",
        '                table=breakdown(rows), notes=notes)',
        '',
        "if __name__ == '__main__':",
        '    build()'
      ].join('\n') },
    { nm:'workflow.yml', sub:'the schedule',
      code:[
        '# monthly-report/workflow.yml — the schedule that keeps the page current',
        'name: monthly-data-analysis-report',
        'on:',
        '  schedule: first monday 08:00    # from the wizard\'s answer',
        'run:',
        '  - python build_report.py',
        '  - nebulas publish .             # same URL, new month',
        'on_failure: retry twice, then alert the owner'
      ].join('\n') },
    { nm:'sources.json', sub:'the reads',
      code:[
        '{',
        '  "local": ["q3_ledger"],',
        '  "external": {',
        '    "api": "none — local data only",',
        '    "credentials": "by reference, never in the folder"',
        '  }',
        '}'
      ].join('\n') }
  ],
  ...
  page:{
    site:'Acme · Reports', nav:['Reports','Sources','Runs'], accent:'Blue',
    title:'Monthly data analysis report',
    sub:'July 2026 · built by the monthly workflow · next run first Monday 08:00',
    stats:[['$3.84M','revenue, July'],['+6.2%','against June'],['Enterprise','top mover']],
    table:{ head:['Segment','Share of revenue','Against June'], meter:1, rows:[
      ['Enterprise', 0.46, '+9.1%'],
      ['Mid-market', 0.31, '+4.4%'],
      ['SMB',        0.18, '−1.2%'],
      ['Services',   0.05, '+0.6%']
    ]},
    body:['July closed at $3.84M, 6.2% over June, and the beat is carried by Enterprise renewals — the third month running. SMB slipped again; the notes flag it for the quarterly review. Every number on this page is recomputed by build_report.py on each run of the workflow.']
  }
};
...
/* --------------------------------------------------------------- projects
   ...
   is depends on what its owner switched on — see openProject in app.js.
     ...
     custom  the project's Customized output — its own section below
             Artifacts on every surface: { ids } of the artifacts filed there.
             Chosen, not everything produced; filing joins the add-artifact
             flow
     pages   what it publishes: each one binds an artifact template (the layout),
...
PROJECTS.forEach(p => {
  ...
  if (!p.code) p.code = [];
  if (!p.pages) p.pages = [];
  /* Every project carries a Customized output section — it exists before
     anything is filed in it. */
