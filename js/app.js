...
function openPageWizard(p){
  pwOn = { p:p, step:-1, answers:[], built:false, busy:false, done:false,
    /* Two ways in: asked for (chat), or written by hand (manual). The manual
       script is a working copy of the fixture, so edits survive tab switches
       without touching what the next wizard starts from. */
    mode:'chat',
    manual:JSON.parse(JSON.stringify(D.PAGE_WIZARD.manual)) };
...
function closePageWizard(){
  ...
  if (w && !w.done && (w.answers.length || w.built))
    toast('Left the page wizard — nothing was filed');
}

/* The chat and the manual script are two tabs over one left column. Chat
   keeps its state while manual is up (the conversation resumes where it
   paused), and both arrive at the same place: a built page on the right. */
function pwSyncMode(){
  const w = pwOn;
  const host = $('#pwMode');
  host.innerHTML = '';
  if (!w) return;
  [['chat','AI Chat'],['manual','Manual Script']].forEach(([k, l]) => {
    const b = el('button','tab', l);
    b.type = 'button';
    b.setAttribute('aria-selected', String(w.mode === k));
    b.onclick = () => { if (pwOn){ pwOn.mode = k; pwSyncMode(); } };
    host.append(b);
  });
  const manual = w.mode === 'manual';
  $('#pwLog').hidden = manual;
  $('#pwAskRow').hidden = manual;
  $('#pwManual').hidden = !manual;
  if (manual) pwManualPane();
}

/* The Manual Script pane: the page as a hand-written definition — declared
   inputs, the tables it reads, the report step as editable SQL — and a test
   row whose Preview is the same act as the chat build: the page exists, the
   right pane shows it. Edits live on the wizard's working copy. */
function pwManualPane(){
  const w = pwOn, m = w.manual, host = $('#pwManual');
  host.innerHTML = '';

  const section = (title, ico) => {
    const s = el('section','pwman__sec');
    s.append(el('div','pwman__head', ic(ico, 13) +
      '<span class="t-eyebrow">' + esc(title) + '</span>'));
    return s;
  };

  const inputs = section('Inputs','filetext');
  m.inputs.forEach(inp => {
    const row = el('div','pwman__row');
    row.append(el('span','pwman__k t-mono', esc(inp.k)));
    row.append(el('span','pwman__hint', esc(inp.hint)));
    row.append(el('span','pwman__eq','='));
    const v = inputCtl(inp.v, val => { inp.v = val; });
    v.classList.add('input--mono');
    row.append(v);
    inputs.append(row);
  });
  host.append(inputs);

  const srcs = section('Data sources','data');
  m.sources.forEach(nm => srcs.append(el('div','pwman__row',
    '<span class="pwman__k t-mono">' + esc(nm) + '</span>')));
  host.append(srcs);

  const rep = section('Report','doc');
  m.steps.forEach(st => {
    const card = el('div','pwman__step');
    card.append(el('div','pwman__stephead',
      '<span class="pwman__n">' + st.n + '</span>' +
      '<span class="pwman__lang">' + esc(st.lang) + '</span>' +
      '<span class="pwman__arrow">→</span>' +
      '<span class="t-mono pwman__out">' + esc(st.out) + '</span>'));
    const code = el('textarea','textarea textarea--code pwman__code');
    code.value = st.code;
    code.spellcheck = false;
    code.oninput = () => { st.code = code.value; };
    card.append(code);
    rep.append(card);
  });
  host.append(rep);

  const test = el('div','pwman__test');
  test.append(el('strong','pwman__testt','Test report generation'));
  const vals = el('div','pwman__vals');
  m.inputs.forEach(inp => {
    vals.append(el('span','pwman__k t-mono', esc(inp.k)));
    const v = inputCtl(inp.v, val => { inp.v = val; });
    v.classList.add('input--mono');
    vals.append(v);
  });
  test.append(vals);
  const run = el('button','btn btn--primary btn--sm', ic('play',13) + 'Preview');
  run.type = 'button';
  run.onclick = () => pwManualPreview();
  test.append(run);
  host.append(test);
}

function pwManualPreview(){
  const w = pwOn;
  if (!w) return;
  ...
  w.built = true;
  pwSyncSide();
  toast('Preview rebuilt from the script');
}
...
/* The right pane: a placeholder until the build, then the preview — the page
   itself under its address. The script has its own homes (the Manual Script
   tab on the left; the artifact's Files tab once filed), so the pane shows
   one thing and shows it whole. */
function pwSyncSide(){
  ...
  if (!w.built){
    side.append(emptyState('template','The page appears here',
      'Answer on the left, one step at a time. When the build finishes, the preview fills this pane.'));
    return;
  }
  const url = pwUrl(w);
